...
const EventGallery: React.FC<EventGalleryProps> = ({ mediaLinks, eventTitle }) => {
  ...
  const nextImage = () => {
    setLightboxIndex((prev) => (prev + 1) % mediaLinks.photos.length);
  };

  const prevImage = () => {
    setLightboxIndex((prev) => (prev - 1 + mediaLinks.photos.length) % mediaLinks.photos.length);
  };
    ...
    <div className="mt-8">
      ...
      {(activeTab === 'photos' || !hasVideos) && hasPhotos && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {mediaLinks.photos.map((photo, index) => (
            <button
              key={index}
      ...
      {/* Videos Grid */}
      {(activeTab === 'videos' || !hasPhotos) && hasVideos && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {mediaLinks.videos.map((video, index) => {
      ...
      {lightboxOpen && hasPhotos && (
        ...
        >
          ...
          {/* Image */}
          <img
            src={mediaLinks.photos[lightboxIndex]}
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white text-sm">
            {lightboxIndex + 1} / {mediaLinks.photos.length}
          </div>
        </div>
      )}
    </div>
